...
Generate ${maxSuggestions} concise, relevant questions (each under 50 characters) that naturally follow from the conversation. Return them as a JSON array of strings.
...
Example format: ["Question 1?", "Question 2?", "Question 3?"]`;
  ...
  try {
    // For now, return some default suggestions based on content analysis
    // In a real implementation, this would call an AI model
    const suggestions = await generateSimpleSuggestions(lastAssistantMessage.content, maxSuggestions);
